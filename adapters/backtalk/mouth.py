...
# Codex Jarvis adaptation: prefetch local TTS to remove inter-sentence gaps.
...
class Mouth:
    def __init__(self):
        ...
        # Text jobs enter _q. A dedicated synthesis worker renders them
        # ahead of playback into _audio_q, so sentence N+1 can be prepared
        # while sentence N is still audible.
        ...
        self._synth_worker = threading.Thread(
            target=self._synth_run, daemon=True, name="tts-prefetch")
        self._worker = threading.Thread(
            target=self._run, daemon=True, name="tts-playback")
    ...
    def say_chunk(self, text: str, directions=None):
        ...
        if text:
            with self._generation_lock:
                generation = self._generation
            self._q.put((text, directions or None, generation))

    def shut_up(self):
        """Barge-in: stop current playback and flush everything queued."""
        self._stop.set()
        # Invalidate anything currently synthesizing and anything already
        # prefetched, then flush both stages.
        with self._generation_lock:
            self._generation += 1
    ...
    def wait_done(self, timeout: float | None = None):
        ...
        while ((not self._q.empty()) or (not self._audio_q.empty())
               or self._speaking.is_set() or self._synthing.is_set()):
            time.sleep(0.05)
    ...
    def _synth_run(self):
        """Render queued speech ahead of playback.

        The original Backtalk worker synthesized a sentence only after the
        ...
        while True:
            ...
            t0 = time.monotonic()
            try:
                chunks = list(synth_stream(sentence))
                synth_ms = int((time.monotonic() - t0) * 1000)
                if generation != self._current_generation():
                    continue
                if chunks:
                    self._audio_q.put(
                        (sentence, directions, generation, chunks, synth_ms))
            except Exception as e:
    ...
    def _run(self):
        ...
        while True:
            sentence, directions, generation, chunks, synth_ms = self._audio_q.get()
            if generation != self._current_generation():
                continue
    ...
    def _play_chunks(self, chunks, directions=None, block: int = 2205):
        """Play already-synthesized PCM immediately.

        Because synthesis happens on the other worker, there is no per-sentence
        prebuffer wait here; the persistent output stream is still reused.
        """
        from backtalk import signals
        if not chunks:
            return
